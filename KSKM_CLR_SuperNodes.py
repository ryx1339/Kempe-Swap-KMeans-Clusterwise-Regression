...
def KSAssignment(k, membership, sub_adjs, D, skip_phases = False):
    ...
    def MWSP(weights,
            ...
            adj_swaps_v_array):
        """
        weights                  : (n,) 1d array-like
        points_in_swaps_array    : 1d int array, concatenated incidence lists
        points_in_swaps_array_index : 1d int array of pointers (CSR-style), length = (#points + 1)
        adj_swaps_array          : (>=len_adj_swaps, 2) int array of conflicting pairs
        len_adj_swaps            : int, number of valid rows in adj_swaps_array to use
        """
        m = gp.Model("MWSP")
